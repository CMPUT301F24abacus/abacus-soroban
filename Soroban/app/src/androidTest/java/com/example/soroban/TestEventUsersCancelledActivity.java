...
public class TestEventUsersCancelledActivity {
    ...
    private static Intent createEventUsersCancelledActivityIntent(User appUser, Event mockEvent) {
        Intent intent = new Intent(Intent.ACTION_MAIN);
        intent.setClassName("com.example.soroban", "com.example.soroban.activity.EventUsersCancelledActivity");
    ...
    public void testConfirmedDisplayed() {
        ...
        mockEvent.addToNotGoing(mockEntrant);
        ActivityScenario<EventUsersCancelledActivity> scenario = ActivityScenario.launch(createEventUsersCancelledActivityIntent(appUser, mockEvent));

        UserList mockList = mockEvent.getNotGoing();
        scenario.onActivity( activity -> {
            UserArrayAdapter mockUserAdapter = new UserArrayAdapter(activity, mockList);
            ListView listView = activity.findViewById(R.id.usersCancelledView);
            listView.setAdapter(mockUserAdapter);
        });

        onView(withId(R.id.usersCancelledView)).check(matches(isDisplayed()));
        onData(is(instanceOf(User.class))).inAdapterView(withId(R.id.usersCancelledView))
                .atPosition(0)
