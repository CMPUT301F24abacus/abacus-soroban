...
public class TestEventUsersConfirmedActivity {
    ...
    private static Intent createEventUsersConfirmedActivityIntent(User appUser, Event mockEvent) {
        Intent intent = new Intent(Intent.ACTION_MAIN);
        intent.setClassName("com.example.soroban", "com.example.soroban.activity.EventUsersConfirmedActivity");
    ...
    public void testConfirmedDisplayed() {
        ...
        mockEvent.addAttendee(mockEntrant);
        ActivityScenario<EventUsersConfirmedActivity> scenario = ActivityScenario.launch(createEventUsersConfirmedActivityIntent(appUser, mockEvent));

        UserList mockList = mockEvent.getAttendees();
        scenario.onActivity( activity -> {
            UserArrayAdapter mockUserAdapter = new UserArrayAdapter(activity, mockList);
            ListView listView = activity.findViewById(R.id.usersFinalListView);
            listView.setAdapter(mockUserAdapter);
        });

        onView(withId(R.id.usersFinalListView)).check(matches(isDisplayed()));
        onData(is(instanceOf(User.class))).inAdapterView(withId(R.id.usersFinalListView))
                .atPosition(0)
